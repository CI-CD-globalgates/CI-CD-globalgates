const joinService = (() => {
        ...
        const checkCompanyName = async (companyName) => {
            const response = await fetch(`/api/member/check-company-name?companyName=${companyName}`);
            const text = await response.text();
        ...
        const checkBusinessNumber = async (businessNumber) => {
            const response = await fetch(`/api/member/check-business-number?businessNumber=${businessNumber}`);
            const text = await response.text();
            if (!response.ok) throw new Error(text || "Fetch error");
            return text === "true";
        }
