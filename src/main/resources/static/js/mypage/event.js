...
window.onload = function () {
    ...
    const productCategoryRecommendation = document.getElementById(
        "productCategoryRecommendation",
    );
    const productCategoryRecommendationStatus = document.getElementById(
        "productCategoryRecommendationStatus",
    );
    const productCategoryRecommendationChips = document.getElementById(
        "productCategoryRecommendationChips",
    );
    let productPreviewUrls = [];
    let productSubmitting = false;
    let recommendationTimer = null;
    let latestCategoryPredictions = [];
    let selectedCategoryName = "";
    let latestRecommendationRequestId = 0;
    ...
    function resetProductForm() {
        ...
        latestCategoryPredictions = [];
        clearTimeout(recommendationTimer);
        renderTags();
        showTopChips();
        resetProductImages();
        renderIdleCategoryRecommendation();
    }
    ...
    function setCategoryRecommendationState(state, message) {
        if (productCategoryRecommendation) {
            productCategoryRecommendation.dataset.state = state;
        }
        if (productCategoryRecommendationStatus) {
            productCategoryRecommendationStatus.textContent = message;
        }
    }

    function getRecommendationRankLabel(index) {
        if (index === 0) return "가장 유력";
        if (index === 1) return "대안";
        return "보조 후보";
    }

    function clearCategoryRecommendationChips() {
        if (productCategoryRecommendationChips) {
            productCategoryRecommendationChips.innerHTML = "";
        }
    }

    function renderCategoryRecommendationChips() {
        if (!productCategoryRecommendationChips) return;

        productCategoryRecommendationChips.innerHTML = latestCategoryPredictions
            .map((prediction, index) => {
                const categoryName = escapeHtml(prediction.categoryName || "");
                const rankLabel = getRecommendationRankLabel(index);
                const isApplied =
                    (prediction.categoryName || "").trim() === selectedCategoryName;

                return `
                    <button
                        type="button"
                        class="Recommend-Chip${index === 0 ? " Recommend-Chip--Primary" : ""}${isApplied ? " Recommend-Chip--Applied" : ""}"
                        data-recommended-category="${categoryName}"
                        aria-pressed="${isApplied ? "true" : "false"}"
                    >
                        <span class="Recommend-Chip-Rank">${rankLabel}</span>
    ...
    function renderIdleCategoryRecommendation() {
        clearCategoryRecommendationChips();
        setCategoryRecommendationState(
            "idle",
            "상품명이나 설명을 입력하면 추천해드려요",
        );
    }

    function findCategoryMatch(categoryName) {
        for (const chip of originalChips) {
            const parentName = (chip.dataset.cat || "").trim();
    ...
    function applyRecommendedCategory(categoryName) {
        const trimmedCategoryName = (categoryName || "").trim();
        if (!trimmedCategoryName) return;
        if (!addTag(trimmedCategoryName, trimmedCategoryName)) return;

        selectedCategoryName = trimmedCategoryName;
        const matchedCategory = findCategoryMatch(trimmedCategoryName);

        if (matchedCategory?.type === "sub") {
            showSubChips(
                matchedCategory.parentName,
                matchedCategory.subs,
                matchedCategory.parentName,
            );
        } else {
            showTopChips();
        ...
        renderCategoryRecommendationChips();
        setCategoryRecommendationState(
            "ready",
            "추천 카테고리를 선택했어요",
        );
    ...
    async function requestCategoryRecommendation() {
        const payload = {
            postTitle: productNameInput?.value.trim() || "",
            postContent: productContentInput?.value.trim() || "",
            postTag: postTagsInput?.value.trim() || "",
        };

        if (!payload.postTitle && !payload.postContent && !payload.postTag) {
            latestCategoryPredictions = [];
            renderIdleCategoryRecommendation();
            return;
        }
        ...
        setCategoryRecommendationState(
            "loading",
            "입력 내용을 바탕으로 카테고리를 추천 중입니다.",
        );

        const requestId = ++latestRecommendationRequestId;

        try {
            const result = await myPageService.getCategoryRecommendation(payload);
            if (requestId !== latestRecommendationRequestId) return;
            ...
            if (!predictions.length) {
                ...
                setCategoryRecommendationState(
                    "empty",
                    "추천 가능한 카테고리를 아직 찾지 못했어요",
                );
                return;
            }

            latestCategoryPredictions = predictions.slice(0, 3);
            renderCategoryRecommendationChips();
            setCategoryRecommendationState(
                "ready",
                "가장 적합한 카테고리를 보여드려요",
    ...
    function scheduleCategoryRecommendation() {
        clearTimeout(recommendationTimer);
        recommendationTimer = setTimeout(requestCategoryRecommendation, 500);
    }
    ...
    productNameInput?.addEventListener("input", scheduleCategoryRecommendation);
    productContentInput?.addEventListener("input", scheduleCategoryRecommendation);
    productImageResetButton?.addEventListener("click", resetProductImages);
    productCategoryRecommendationChips?.addEventListener("click", (e) => {
        const chip = e.target.closest("[data-recommended-category]");
        if (!chip) return;

        applyRecommendedCategory(chip.dataset.recommendedCategory || "");
    });
    ...
    function addTag(label, chipKey) {
        const t = label.trim();
        if (!t) return false;
        if (selectedTags.some((x) => x.chipKey === chipKey)) return true;
    ...
    function showTopChips() {
        if (!categoryScroll) return;
        chipViewState = "top";
        delete categoryScroll.dataset.parentName;
        ...
        originalChips.forEach((chip) => {
            ...
            const hasSubSelected =
                subs.length > 0 && subs.includes(selectedCategoryName);
            const selfSelected =
                subs.length === 0 && chip.dataset.cat === selectedCategoryName;
    ...
    function showSubChips(parentName, subsArray, parentChipKey) {
        if (!categoryScroll) return;
        chipViewState = "sub";
        categoryScroll.dataset.parentName = parentName;
        categoryScroll.innerHTML = "";
    ...
    categoryScroll?.addEventListener("click", (e) => {
        ...
        if (chipViewState === "sub") {
            ...
            const isActive = selectedCategoryName === ck;
            if (isActive) {
                selectedCategoryName = "";
                removeTagByKey(ck);
            } else {
                if (!addTag(ck, ck)) return;
                selectedCategoryName = ck;
            }
            categoryScroll
                .querySelectorAll(".Cat-Chip--Sub-Active")
                .forEach((subChip) =>
                    subChip.classList.remove("Cat-Chip--Sub-Active"),
                );
            if (selectedCategoryName) {
                chip.classList.add("Cat-Chip--Sub-Active");
            }
            renderCategoryRecommendationChips();
            return;
        }
